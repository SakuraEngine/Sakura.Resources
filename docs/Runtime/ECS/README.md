...
在 **Dual**，我们使用了组块方案作为 ECS 的对象模型。

## API 设计

* 面向数据：在 **Dual** 中，我们遵循了面向数据的原则，弱化了对象的存在，而增强了数据的存在。具体来说，几乎所有 API 操作的目标都不是对象，而是数据：在 **Dual** 中为 `**Dual**_chunk_view_t`，遍历时产生 `**Dual**_chunk_view_t`，处理 entity 时产生 `**Dual**_chunk_view_t`，生成 entity 时产生 `**Dual**_chunk_view_t` 等等。
* 批量化：在面向数据设计的基础上，**Dual** 尽量进行了批量化处理，比如在处理 entity 列表时，会先尝试将其合批为 `**Dual**_chunk_view_t` 再进行具体的操作，在操作类型的时候（增删组件），尽量只进行一步操作：`Add(A,B)`　而不是 `Add(A), Add(B)`
